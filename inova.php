...
$id = $_POST['id'];
  ...
  if($voto>0){
  	  if($votoan == 1){
      $sqlupdate = mysqli_query($con,"UPDATE INSIGNIAS SET insignia=1,puntos='4' WHERE email = '$email' and id_emprendimiento ='$id'") or die ("Error de conexión");
      ...
      $coins=mysqli_fetch_array($suma);
      $educacoins=$coins['SUM(puntos)'];
	  $arr = array('inova' => $conta, 'puntos' => $educacoins, 'borra' => '2');
	  echo json_encode($arr);
	  }
	  else if($votoan==3){
	  	$sqlupdate = mysqli_query($con,"UPDATE INSIGNIAS SET insignia=1,puntos='4' WHERE email = '$email' and id_emprendimiento ='$id'") or die ("Error de conexión");
      ...
      $registra = mysqli_query($con,"INSERT INTO INSIGNIAS VALUES (NULL,'$email','$id',1,4,NOW())") or die ("Error de conexión");
      $contador = mysqli_query($con,"SELECT id FROM INSIGNIAS WHERE id_emprendimiento='$id' and insignia=1") or die ("Error de conexión");
      $conta = mysqli_num_rows($contador);
      $suma=mysqli_query($con,"SELECT SUM(puntos) FROM INSIGNIAS where id_emprendimiento=$id") or die ("Error de conexión");
      $coins=mysqli_fetch_array($suma);
