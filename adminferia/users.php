            ...
            <div class="table-responsive">
              <table class="table align-items-center table-flush">
                ...
                <tbody class="list">
                  ...
                  <?
                    $datos=mysqli_query($con,"SELECT nombre,apellidos,email,DATE_SUB(fecha_registro, INTERVAL 5 HOUR) FROM REGISTRO") or die ("Error de conexión");
                    while ($row = mysqli_fetch_array($datos)) {
                    $nombre=$row['nombre'];
                    $apellidos=$row['apellidos'];
                    $email=$row['email'];
                    $fecha_registro=$row['DATE_SUB(fecha_registro, INTERVAL 5 HOUR)'];
                      echo'
                  <tr>
                    <th scope="row">
